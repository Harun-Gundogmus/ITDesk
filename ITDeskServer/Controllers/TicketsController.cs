...
//                using (var stream = System.IO.File.Create(@"C:\Angular\ITDesk\ITDeskClient\src\assets" + fileName))
//                {
//                    file.CopyTo(stream);
//                }
...
//        _context.TicketDetais.Add(ticketDetail);
...
//    {
//        var details = _context.TicketDetais
...
//    {
//        Ticket? ticket = _context.Tickets.Where(p => p.Id == request.TicketId).FirstOrDefault();
...
public class TicketsController : ApiController
{
    ...
    [HttpGet("{ticketId}")]
    public IActionResult GetDetails(Guid ticketId) //api/Tickets/GetDetails?ticketId=asdgf gibi bekler
    {
        var details =
            _context.TicketDetais
            .Where(p => p.TicketId == ticketId)
            .Include(p => p.AppUser)
            .OrderBy(p => p.CreatedDate)
            .ToList();
    ...
    public IActionResult GetAll(GetAllTicketDto request)
    {
        // Kullanıcının rolüne göre listeyi getirir.
        string? userId = HttpContext.User.Claims.Where(p => p.Type == "UserId").Select(s => s.Value).FirstOrDefault();
        if (userId is null)
        {
            return BadRequest(new { Message = "Kullanıcı bulunmadı!" });
